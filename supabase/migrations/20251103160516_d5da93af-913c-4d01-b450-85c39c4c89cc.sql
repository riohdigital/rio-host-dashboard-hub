-- Criar nova função fn_get_cleaner_reservations_for_payment para relatório de pagamentos
-- Esta função filtra por check_in_date (quando a faxina foi realizada)
CREATE OR REPLACE FUNCTION public.fn_get_cleaner_reservations_for_payment(
  start_date date DEFAULT '1900-01-01'::date, 
  end_date date DEFAULT '2099-12-31'::date, 
  property_ids uuid[] DEFAULT NULL::uuid[]
)
RETURNS TABLE(
...
BEGIN
  ...
  ORDER BY rd.check_in_date DESC;
END;
$function$;