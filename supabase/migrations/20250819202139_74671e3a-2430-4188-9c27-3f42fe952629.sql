...
ALTER TABLE public.alerts_destination_property_links ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.anfitriao_alerta_chat_history ENABLE ROW LEVEL SECURITY; 
...
-- Políticas para alerts_destination_property_links
CREATE POLICY "Masters can manage alerts destination property links" 
ON public.alerts_destination_property_links 
FOR ALL 
USING (get_current_user_role() = 'master');

-- Políticas para anfitriao_alerta_chat_history  
CREATE POLICY "Masters can manage chat history"
ON public.anfitriao_alerta_chat_history
FOR ALL
USING (get_current_user_role() = 'master');

-- Políticas para notification_destinations
CREATE POLICY "Masters can manage notification destinations"
ON public.notification_destinations  
FOR ALL
USING (get_current_user_role() = 'master');

-- Políticas para sent_alerts
CREATE POLICY "Masters can view sent alerts"
ON public.sent_alerts
FOR SELECT  
USING (get_current_user_role() = 'master');

-- Corrigir search_path nas funções criadas anteriormente
