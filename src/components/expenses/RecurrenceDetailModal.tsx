...
const ExpensesList = () => {
  ...
  const groupedExpenses = useMemo(() => {
    ...
    return { 
      recurrent: Object.values(recurrentGroups).map(g => ({...g, count: g.expenses.length})), 
      single: singleExpenses 
    };
  }, [expenses]);
  ...
  const handleEditGroup = (expense: Expense) => {
    setDetailModalOpen(false); // Fecha o modal de detalhes
    // Espera um pouco para a UI atualizar antes de abrir o outro modal
    setTimeout(() => {
        setEditingExpense(expense);
        setDialogOpen(true);
    }, 100);
  ...
  const handleMarkAsPaid = async (expenseId: string) => {
    try {
        ...
        fetchExpenses();
        setDetailModalOpen(false); // Fecha e reabre para atualizar a lista
        setTimeout(() => setDetailModalOpen(true), 100);
    } catch (error: any) {
        toast({ title: "Erro", description: `Não foi possível marcar como pago: ${error.message}`, variant: "destructive" });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Dialog open={dialogOpen} onOpenChange={(open) => { if (!open) setEditingExpense(null); setDialogOpen(open); }}>
          <DialogTrigger asChild><Button onClick={() => setEditingExpense(null)} className="bg-primary hover:bg-primary/90 text-white"><Plus className="h-4 w-4 mr-2" />Nova Despesa</Button></DialogTrigger>
          <DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>{editingExpense ? 'Editar' : 'Nova'} Despesa</DialogTitle></DialogHeader><ExpenseForm expense={editingExpense} onSuccess={handleSuccess} onCancel={() => setDialogOpen(false)} /></DialogContent>
        </Dialog>
      </div>
